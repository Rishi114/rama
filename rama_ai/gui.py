"""
RAMA AI - WPF GUI Application
Modern Dark Theme Desktop UI
"""

import sys
import os

# Add path
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

try:
    import tkinter as tk
    from tkinter import ttk
except ImportError:
    print("Tkinter not available, using fallback")
    import sys
    sys.exit(1)


class RAMAApp:
    """RAMA AI Desktop Application with Dark Theme"""
    
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("🤖 RAMA AI v2.0")
        self.root.geometry("900x650")
        self.root.configure(bg="#1a1a2e")
        
        # Colors
        self.bg_dark = "#1a1a2e"
        self.bg_medium = "#16213e"
        self.bg_light = "#0f3460"
        self.accent = "#e94560"
        self.text_white = "#ffffff"
        self.text_gray = "#a0a0a0"
        
        # Setup UI
        self._setup_ui()
        
        # RAMA instance
        self.rama = None
        
    def _setup_ui(self):
        """Create the UI"""
        
        # Header
        header = tk.Frame(self.root, bg=self.bg_medium, height=60)
        header.pack(fill="x")
        header.pack_propagate(False)
        
        title = tk.Label(
            header,
            text="🤖 RAMA AI - Your Desi Voice Assistant",
            font=("Segoe UI", 18, "bold"),
            bg=self.bg_medium,
            fg=self.text_white
        )
        title.pack(side="left", padx=20, pady=15)
        
        # Status indicator
        self.status = tk.Label(
            header,
            text="● Online",
            font=("Segoe UI", 10),
            bg=self.bg_medium,
            fg="#00ff88"
        )
        self.status.pack(side="right", padx=20)
        
        # Main content area
        main = tk.Frame(self.root, bg=self.bg_dark)
        main.pack(fill="both", expand=True, padx=10, pady=10)
        
        # Left sidebar - Skills
        sidebar = tk.Frame(main, bg=self.bg_medium, width=200)
        sidebar.pack(side="left", fill="y", padx=(0, 10))
        sidebar.pack_propagate(False)
        
        # Sidebar title
        tk.Label(
            sidebar,
            text="🎯 Skills",
            font=("Segoe UI", 12, "bold"),
            bg=self.bg_medium,
            fg=self.text_white
        ).pack(pady=(15, 10))
        
        # Skills list - Full 50+ skills
        skills = [
            "💬 Chat", "💻 Coding", "🧮 Calculator", "📁 Files", "💻 System",
            "🔍 Search", "🌤️ Weather", "🎤 Voice", "🧠 Learning",
            "🔧 JSON Repair", "📦 Vercel Skills", "📺 Video Learning",
            "🐛 Debugger", "🏗️ Architect", "🔄 Refactor", "🤖 Agent",
            "⚙️ Automation", "🧠 Context", "📖 RAG", "🎬 Video",
            "⚡ Emotion", "🔒 Security", "🧪 Testing", "🕸️ Swarm",
            "🌐 Web Reach", "📝 Notes", "⏰ Reminders", "🚀 App Launcher",
            "🔧 Advanced", "📚 Knowledge", "👤 Profile", "📊 Memory"
        ]
        
        for skill in skills:
            tk.Label(
                sidebar,
                text=skill,
                font=("Segoe UI", 10),
                bg=self.bg_medium,
                fg=self.text_gray,
                anchor="w"
            ).pack(fill="x", padx=15, pady=3)
        
        # Middle - Chat area
        chat_frame = tk.Frame(main, bg=self.bg_medium)
        chat_frame.pack(side="left", fill="both", expand=True)
        
        # Chat title
        tk.Label(
            chat_frame,
            text="💬 Conversation",
            font=("Segoe UI", 12, "bold"),
            bg=self.bg_medium,
            fg=self.text_white
        ).pack(pady=(15, 5), padx=15, anchor="w")
        
        # Chat display
        self.chat_text = tk.Text(
            chat_frame,
            bg=self.bg_dark,
            fg=self.text_white,
            font=("Segoe UI", 11),
            wrap="word",
            state="disabled",
            bd=0,
            highlightthickness=0
        )
        self.chat_text.pack(fill="both", expand=True, padx=15, pady=5)
        
        # Configure chat text tags
        self.chat_text.tag_config("user", foreground="#4da6ff")
        self.chat_text.tag_config("rama", foreground="#ff6b6b")
        self.chat_text.tag_config("system", foreground="#a0a0a0")
        
        # Input area
        input_frame = tk.Frame(chat_frame, bg=self.bg_dark)
        input_frame.pack(fill="x", padx=15, pady=(5, 15))
        
        self.input_box = tk.Entry(
            input_frame,
            bg=self.bg_medium,
            fg=self.text_white,
            font=("Segoe UI", 12),
            bd=0,
            highlightthickness=1,
            highlightcolor=self.accent,
            insertbackground=self.text_white
        )
        self.input_box.pack(side="left", fill="x", expand=True, padx=(0, 10))
        self.input_box.bind("<Return>", self._send_message)
        
        send_btn = tk.Button(
            input_frame,
            text="Send ➤",
            bg=self.accent,
            fg=self.text_white,
            font=("Segoe UI", 10, "bold"),
            bd=0,
            padx=15,
            pady=5,
            cursor="hand2",
            command=self._send_message
        )
        send_btn.pack(side="right")
        
        # Voice button
        voice_btn = tk.Button(
            input_frame,
            text="🎤",
            bg=self.bg_medium,
            fg=self.text_white,
            font=("Segoe UI", 12),
            bd=0,
            padx=10,
            cursor="hand2"
        )
        voice_btn.pack(side="right", padx=(5, 0))
        
        # Right sidebar - Info
        info_sidebar = tk.Frame(main, bg=self.bg_medium, width=180)
        info_sidebar.pack(side="right", fill="y", padx=(10, 0))
        info_sidebar.pack_propagate(False)
        
        # Info title
        tk.Label(
            info_sidebar,
            text="ℹ️ Info",
            font=("Segoe UI", 12, "bold"),
            bg=self.bg_medium,
            fg=self.text_white
        ).pack(pady=(15, 10))
        
        # Info labels
        info_items = [
            ("👤 Name:", "bhai"),
            ("🗣️ Language:", "English"),
            ("🎭 Mood:", "Sassy"),
            ("💾 Memory:", "Active"),
            ("🔗 KiloCode:", "Ready"),
            ("🎤 Voice:", "Enabled"),
        ]
        
        for label, value in info_items:
            frame = tk.Frame(info_sidebar, bg=self.bg_medium)
            frame.pack(fill="x", padx=10, pady=2)
            
            tk.Label(
                frame,
                text=label,
                font=("Segoe UI", 9),
                bg=self.bg_medium,
                fg=self.text_gray
            ).pack(side="left")
            
            tk.Label(
                frame,
                text=value,
                font=("Segoe UI", 9, "bold"),
                bg=self.bg_medium,
                fg=self.text_white
            ).pack(side="right")
        
        # Bottom status bar
        status_bar = tk.Frame(self.root, bg=self.bg_medium, height=30)
        status_bar.pack(fill="x")
        status_bar.pack_propagate(False)
        
        tk.Label(
            status_bar,
            text="RAMA AI v2.0 | 50+ Skills | 35+ Languages | KiloCode + openclaw + JSON Repair + Vercel Skills + Video Learning",
            font=("Segoe UI", 9),
            bg=self.bg_medium,
            fg=self.text_gray
        ).pack(pady=5)
        
    def _send_message(self, event=None):
        """Handle message sending"""
        message = self.input_box.get().strip()
        if not message:
            return
        
        # Clear input
        self.input_box.delete(0, "end")
        
        # Display user message
        self._add_message(f"You: {message}", "user")
        
        # Get RAMA response
        if not self.rama:
            self._init_rama()
        
        response = self.rama.process(message)
        
        # Display response
        self._add_message(f"Rama: {response}", "rama")
        
    def _add_message(self, text, tag):
        """Add message to chat"""
        self.chat_text.config(state="normal")
        self.chat_text.insert("end", text + "\n\n", tag)
        self.chat_text.see("end")
        self.chat_text.config(state="disabled")
    
    def _init_rama(self):
        """Initialize RAMA"""
        try:
            from main import RAMAAI
            self.rama = RAMAAI()
            self._add_message("🤖 RAMA AI initialized! Ask me anything!", "system")
        except Exception as e:
            self._add_message(f"❌ RAMA init error: {e}", "system")
    
    def run(self):
        """Run the app"""
        self.root.mainloop()


# Main entry
if __name__ == "__main__":
    app = RAMAApp()
    app.run()